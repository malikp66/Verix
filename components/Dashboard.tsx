...
function HeroSection({ setActiveTab }: { setActiveTab: (id: string) => void }) {
  return (
    <section className="relative pt-32 pb-32 flex flex-col items-center justify-center text-center px-4 overflow-hidden min-h-[95vh]">
      ...
      <div className="absolute inset-0 bg-[#0A0E13] -z-20" />
      <div className="absolute inset-0 bg-[radial-gradient(ellipse_100%_100%_at_50%_0%,_rgba(16,185,129,0.1)_0%,_rgba(10,14,19,1)_80%)] -z-10" />
      
      {/* Topology / Radar lines subtle background */}
      <div className="absolute inset-0 opacity-15 -z-10 bg-[linear-gradient(to_right,#ffffff_1px,transparent_1px),linear-gradient(to_bottom,#ffffff_1px,transparent_1px)] bg-[size:4rem_4rem] [mask-image:radial-gradient(ellipse_60%_60%_at_50%_30%,#000_20%,transparent_100%)]" />

      {/* Floating threat nodes simulated with gradients */}
      <div className="absolute top-1/4 left-1/4 w-96 h-96 bg-emerald-500/10 rounded-full blur-[120px] -z-10 mix-blend-screen animate-[pulse_8s_ease-in-out_infinite]" />
      <div className="absolute bottom-1/4 right-1/4 w-[30rem] h-[30rem] bg-indigo-500/5 rounded-full blur-[150px] -z-10 mix-blend-screen animate-[pulse_10s_ease-in-out_infinite]" />
      ...
      <motion.div
        ...
        className="mt-20 w-full max-w-2xl bg-[#121821]/40 backdrop-blur-sm border border-neutral-800/80 rounded-2xl p-6 text-left relative overflow-hidden"
      >
        <div className="absolute top-0 right-0 p-4 opacity-10 pointer-events-none">
          <Zap className="w-24 h-24 text-emerald-500" />
        </div>
        <div className="flex items-center gap-2 mb-3 relative z-10">
          <span className="text-[10px] font-mono text-emerald-400 tracking-widest bg-emerald-500/10 px-2 py-1 rounded">DAILY AI BRIEFING</span>
        </div>
        <p className="text-neutral-300 font-medium leading-relaxed relative z-10 text-lg">
          &quot;Scam berbasis <span className="text-white bg-neutral-800 px-1 rounded">fake customer service marketplace</span> meningkat drastis menjelang periode gajian. Waspadai nomor berawalan +1 atau +62 85 yang mengirimkan link resolusi masalah.&quot;
        </p>
      </motion.div>
    </section>
  );
...
function LiveScamRadar() {
  ...
  const fallbackAlerts = [
    { title: 'Fake QRIS di Cafe Jaksel', changePct: 42, type: 'critical', time: '10m ago', valueType: 'threat' },
    { title: 'APK Kurir J&T Viral', changePct: 800, type: 'danger', time: '23m ago', valueType: 'victims' },
    { title: 'Voice Cloning (Deepfake) Suami/Istri', changePct: 15, type: 'warning', time: '1h ago', valueType: 'new' },
    { title: 'Telegram Crypto Investment', changePct: 89, type: 'warning', time: '2h ago', valueType: 'surging' }
  ];

  const alerts = intel?.dashboardAlerts || fallbackAlerts;

  return (
    <section className="py-12 bg-[#0A0A0A] border-b border-neutral-900 overflow-hidden">
      <div className="max-w-7xl mx-auto px-6 h-full flex flex-col md:flex-row gap-8 items-center">
        <div className="flex-shrink-0 w-64 border-r border-neutral-900 pr-8">
          <p className="text-[10px] font-mono text-neutral-500 mb-1">RADAR TELEMETRY</p>
          <div className="flex items-center gap-2 mb-4">
            <div className="w-3 h-3 rounded-full bg-emerald-500 shadow-[0_0_10px_rgba(16,185,129,0.5)] animate-pulse" />
            <h3 className="text-lg font-medium text-white tracking-tight">Active Threats</h3>
          </div>
          <div className="p-3 bg-neutral-900 rounded-xl border border-neutral-800">
            <p className="text-[10px] font-mono text-neutral-500 mb-1 block">NATIONAL THREAT LEVEL</p>
            <div className="flex items-center gap-2">
              <span className="text-xl font-display font-medium text-amber-500 tracking-tight">ELEVATED</span>
              <AlertCircle className="w-4 h-4 text-amber-500" />
            </div>
          </div>
        </div>
        
        <div className="flex-1 flex gap-6 overflow-x-auto pb-4 md:pb-0 hide-scrollbar w-full relative">
          {alerts.map((alert: any, i: number) => (
            <motion.div 
              key={i} 
              layout
              className="flex-shrink-0 flex items-start gap-4 min-w-[240px] pl-4 border-l border-neutral-900/50"
            >
              <div className="mt-1">
                {alert.type === 'critical' ? <ShieldAlert className="w-4 h-4 text-red-500" /> : 
                 alert.type === 'danger' ? <FileWarning className="w-4 h-4 text-amber-500" /> : 
                 <Radar className="w-4 h-4 text-blue-500" />}
              </div>
              <div>
                <p className="text-sm font-medium text-neutral-200 mb-1">{alert.title}</p>
                <div className="flex items-center gap-3 text-xs">
                  <span className={`${alert.type === 'critical' ? 'text-red-400' : 'text-neutral-400'} font-mono`}>
                    {alert.valueType === 'threat' ? `+${alert.changePct}% threat` : 
                     alert.valueType === 'victims' ? `${alert.changePct}+ victims` : 
                     alert.valueType === 'new' ? `+${alert.changePct}% coverage` : 
                     `+${alert.changePct} cases`}
                  </span>
                  <motion.span 
                    key={alert.time}
                    initial={{ opacity: 0.5, color: '#10b981' }} 
                    animate={{ opacity: 1, color: '#525252' }} 
          ...
          <div className="space-y-3 pt-2">
            {demos.map((demo, idx) => (
              <button
                ...
                className={`w-full text-left px-6 py-5 rounded-2xl transition-all flex justify-between items-center group ${
                  activeDemo === idx 
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
           <div className="md:col-span-3 bg-neutral-950 border border-neutral-800 p-8 rounded-3xl h-[400px] relative overflow-hidden flex items-center justify-center">
             ...
             <div className="absolute inset-4 z-0 flex items-center justify-center overflow-hidden mix-blend-screen opacity-90">
               <ComposableMap
               ...
               >
                 <Geographies geography={geoUrl}>
                   {({ geographies }) =>
                     geographies.map((geo) => {
                       const isIdn = geo.properties.name === "Indonesia" || geo.id === "360";
                       return (
                         <Geography
                           key={geo.rsmKey}
                           geography={geo}
                           fill={isIdn ? "#161D27" : "#0B0F14"}
                           stroke={isIdn ? "#B6FF3B" : "#263041"}
                           strokeWidth={isIdn ? 1 : 0.5}
                           style={{
                 ...
                 {/* Simulated Hotspots over Indonesia */}
                 <Marker coordinates={[106.8456, -6.2088]}> {/* Jakarta */}
                   <circle r={12} fill="#FF5D5D" className="opacity-20 animate-pulse" />
                   <circle r={4} fill="#FF5D5D" className="animate-pulse" />
                   <text textAnchor="middle" y={-10} style={{ fontFamily: "monospace", fontSize: "10px", fill: "#FF5D5D" }}>JKT</text>
                 </Marker>

                 <Marker coordinates={[112.7521, -7.2504]}> {/* Surabaya */}
                   <circle r={8} fill="#FFB547" className="opacity-20 animate-pulse delay-100" />
                   <circle r={3} fill="#FFB547" className="animate-pulse delay-100" />
                 </Marker>

                 <Marker coordinates={[104.7566, -2.9909]}> {/* Palembang */}
                   <circle r={16} fill="#B6FF3B" className="opacity-10 animate-pulse delay-300" />
                   <circle r={3} fill="#B6FF3B" className="animate-pulse delay-300" />
                 </Marker>
               </ComposableMap>
             </div>
